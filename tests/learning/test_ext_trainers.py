...
from learning.ext_sequential_trainer import *
...
env = load_isaaclab_env(
    ...
    headless=eval_cpt#, 
    #cli_args=[f"--episode_length_s {.02*250}"]
)
...
memory = RandomMemory(memory_size=250, num_envs=env.num_envs, device=device)
...
cfg_trainer = {"timesteps": 500, "headless": eval_cpt}
trainer = ExtSequentialTrainer(cfg=cfg_trainer, env=env, agents=agent)
